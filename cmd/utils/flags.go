...
import (
	...
	"github.com/virbicoin/go-virbicoin/consensus/clique"
	...
	"github.com/virbicoin/go-virbicoin/eth"
	...
	"github.com/virbicoin/go-virbicoin/eth/tracers"
	"github.com/virbicoin/go-virbicoin/ethdb"
	"github.com/virbicoin/go-virbicoin/ethstats"
	"github.com/virbicoin/go-virbicoin/graphql"
	...
	"github.com/virbicoin/go-virbicoin/metrics/exp"
	"github.com/virbicoin/go-virbicoin/metrics/influxdb"
...
// for Gvbc and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gvbc account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) ethapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
		...
		if err != nil {
			Fatalf("Failed to register the VIrBiCoin service: %v", err)
		}
...
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the VIrBiCoin Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gvbc.", tagsMap)
		}
...
// new format.
//
// e.g. gvbc account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gvbc --keystore /tmp/mykeystore --lightkdf account new
//

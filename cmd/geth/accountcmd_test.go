...
func TestAccountListEmpty(t *testing.T) {
	gvbc := runGeth(t, "--nousb", "account", "list")
	gvbc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gvbc := runGeth(t, "--nousb", "account", "list", "--datadir", datadir)
	defer gvbc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gvbc := runGeth(t, "account", "new", "--lightkdf")
	defer gvbc.ExpectExit()
	gvbc.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gvbc := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gvbc.ExpectExit()
	gvbc.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gvbc := runGeth(t, "--nousb", "account", "update",
...
func TestWalletImport(t *testing.T) {
	gvbc := runGeth(t, "--nousb", "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gvbc.ExpectExit()
	gvbc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gvbc.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gvbc := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gvbc.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gvbc.SetTemplateFunc("keypath", func(file string) string {

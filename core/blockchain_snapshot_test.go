...
import (
	...
	"github.com/virbicoin/go-virbicoin/core/state/snapshot"
...
// It can be used for testing that restart Gvbc normally.
...
// It can be used for testing that restart Gvbc after the crash.
...
// Tests a Gvbc restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Gvbc restart with valid but "legacy" snapshot. Before the shutdown,
...
// Tests a Gvbc was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Gvbc was crashed and restarts with a broken snapshot. In this case
...
// Tests a Gvbc was crashed and restarts with a broken and "legacy format"
...
// Tests a Gvbc was running with snapshot enabled. Then restarts without
...
// Tests a Gvbc was running with leagcy snapshot enabled. Then restarts
...
// Tests the Gvbc was running with snapshot enabled and resetHead is applied.
...
// Tests the Gvbc was running with snapshot(legacy-format) enabled and resetHead
...
// Tests the Gvbc was running with snapshot(legacy-format) enabled and upgrades
// the disk layer journal(journal generator) to latest format. After that the Gvbc
// is restarted from a crash. In this case Gvbc will find the new-format disk layer
...
// Tests the Gvbc was running with a complete snapshot and then imports a few
